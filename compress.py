...
# -*- encoding: utf-8 -*-
#
# @Description: Python3 压缩与解压缩(zlib / gzip / bz2 / lzma / zipfile / tarfile)
...
# https://docs.python.org/zh-cn/3/library/zipfile.html#module-zipfile
import zipfile
...
def zip_func():
    ...
    # --- class zipfile.ZipFile // 读写的zip类 ---
    # file:文件名 / 类文件对象, mode:r, w, a, x(创建并写入)
    # , compression:ZIP_STORED, ZIP_DEFLATED, ZIP_BZIP2, ZIP_LZMA, allowZip64:zip文件大于2G时需要使用, 支持with
...
def tarfile_func():
    ...
    # mode: r, a, w, x, fileobj:file-obj, format:USTAR_FORMAT,GNU_FORMAT,PAX_FORMAT
    # , tarinfo:可替换默认的TarInfo, dereference:True添加文件,False添加软硬链接
    # , ignore_zeros: 是否忽略空块(损坏的文件设为False), debug:[0,3]
    # class tarfile.TarFile(name=None, mode='r', fileobj=None, format=DEFAULT_FORMAT
    # , tarinfo=TarInfo, dereference=False, ignore_zeros=False, encoding=ENCODING, errors='surrogateescape'
    ...
    # name: 文件名, arcname:存储的成员文件名, exclude: def exclude(filename){已弃用}:return True排除,False添加
    # , filter:def filter(tarinfo):return Tarinfo添加,None排除
